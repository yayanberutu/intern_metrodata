...
define(['N/record', 'N/search'], function (record, search) {
    ...
    function getLatestNumber(format){
    ...
    */
    function createNewestNumbering(format, latest_number, padding){
        ...
        var numberInChar = latest_number.toString().slice(format.toString().length);
    ...
    function beforeSubmit(context) {
        if(context.type == 'create'){
            ...
            var savedsearch = getLatestNumber(format);
            var latest_number = savedsearch[0].getValue('altname');
