...
define(['N/file', "N/ui/serverWidget", "../lib/moment.min.js", "N/search", "N/runtime", "N/url", "N/redirect", "N/record", "N/ui/dialog"],
    function (file, serverWidget, moment, search, runtime, url, redirect, record, dialog) {
        ...
        */
        ...
        function cetakFile(tipe, rawContent, dest, date, norek, context){
            var nameFile = '[' + tipe +']' + dest + date + '_' + norek + '.csv';
            ...
            */
            ...
            context.response.writePage(form);
                ...
                */
                ...
                //check if werite check of bill payment
                var type = "";
                var tipe_file = "";
                if(transType == 1){
                    type = "Check";
                ...
                filters.push(search.createFilter({ name : "type", operator: search.Operator.ANYOF, values: type}));
                ...
                */
                ...
                if(tranSearchResult.length > 0){    
                    ...
                    for(var i=0; i<tranSearchResult.length; i++){
                        ...
                        log.debug("MEMO", memo);
                        log.debug("TYPE OF MEMO", typeof(memo));
                        if(memo == "0"){
                            memo = " ";
                        }
            ...
            cetakFile(tipe_file, rawContent,dest,date, debet_source, context);
